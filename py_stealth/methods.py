...
from .config import STEALTH_CODEC as _STEALTH_CODEC
...
def _ddt2pdt(ddt):  # delphi time into py datetime
    epoch = _datetime.datetime(1899, 12, 30)
    return epoch + _datetime.timedelta(days=ddt)


def _pdt2ddt(pydt):  # py datetime into delphi time
    epoch = _datetime.datetime(1899, 12, 30)
    delta = pydt - epoch
...
_equip_itemsset_macro = _ScriptMethod(357)  # SCEquipItemsSetMacro

def EquipItemsSetMacro():
    _equip_itemsset_macro()
